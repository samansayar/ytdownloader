        ...
        <div className="text-sm relative z-50" dir="ltr">
            <Combobox value={selected} onChange={setSelected}>
                ...
                focus:border-gray-300 dark:focus:border-slate-700 transition text-md placeholder:text-base w-[600px] duration-200 dark:border-slate-700 border-gray-300 pl-4 pr-10"
                        ...
                        />
                        <Combobox.Button onClick={handleSubmitSearchEngine} className="absolute w-14 rounded-r-lg flex justify-center items-center h-full right-0 top-0 bottom-0 dark:bg-slate-700 bg-gray-100 border-gray-300 dark:border-slate-700 border">
                            <IconSearch />
                        </Combobox.Button>
                    </div>
                    <Transition
                            ...
                            {filteredPeople.length === 0 && query !== '' ? (
                                ...
                                filteredPeople.map((person) => (
                                    <Combobox.Option
                                        key={person.id}
                                        className={({ active }) =>
                                            `relative cursor-pointer rounded-md select-none py-2 px-4 ${active ? 'bg-red-500 text-white' : 'text-gray-900'
                                            }`
                                        }
                                        value={person}
                                    >
