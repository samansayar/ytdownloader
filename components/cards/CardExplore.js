...
export default function CardExplore({ data, key }) {
    console.log("DATAFIRSR:::", !data?.main_category_info, data?.id)
    if (data?.main_category_info) {
        return (
            <Link href={`category/${data?.main_category_info?.id}/${data?.main_category_info?.category_title}`} key={key}>
                <div className='capitalize rounded-md text-sm dark:bg-slate-700 bg-slate-50 py-3 px-5 transition duration-150 hover:bg-gray-300 cursor-pointer flex justify-center items-start flex-col h-24'>
                    ...
                    <p className='font-medium text-sm text-left dark:text-slate-100 text-gray-700 mt-1'>{data?.main_category_info?.category_title}</p>
                </div>
            </Link>
        )
    }
