...
export default function Sidebar({ hidden = false }) {
    ...
    if (!hidden || channel && category) {
        return (
            <div className='w-[18%] pb-20 dark:bg-slate-800 h-screen overflow-y-scroll scrollbar-hide hidden lg:flex flex-col pr-2'>
                <ItemMenu />
                ...
                <div className='p-3 flex justify-center flex-col'>
                    <h1 className='dark:text-slate-200 text-md font-medium mx-1'>Subscriptions</h1>
                ...
                <div className='mt-2 px-2'>
                    {category?.map((res, index) => (
                        <Link key={index} href={`/category/${res?.id}/${res?.category_title}`}>
